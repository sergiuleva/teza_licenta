...
void setup() {
  Serial.begin(9600); // Match the baud rate with STM32
...
{
  return firebase.getString(id+"/name");
}

float getWater(String id)
{
  return firebase.getString(id+"/water");
}

void setWater(String id,float water)
{
  firebase.setFloat(id+"/water", float data);
}


int registerUser(String id)
{
  //if user already registered retun -1 else return 0
  //query for user with id
  //if found return -1 else

  firebase.setString(id+"/name", "No name ¯\(º_o)/¯");
  return 0;
}
...
future methods to be implemented here 
